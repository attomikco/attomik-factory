...
export default function NewClientPage() {
  ...
  const handleGenerate = async () => {
    ...
    setGenStep(1);
    try {
      // Simulate step progression (Claude runs all at once, but show progress)
                ...
                {['Analyzing brand colors...', 'Generating store copy...', 'Building homepage config...'].map((label, i) => (
                  <div key={i} className="flex items-center gap-2">
                    {genStep > i + 1 ? (
                      ...
                      <span className="inline-block h-2.5 w-2.5 animate-pulse border border-accent" />
                    ) : (
          ...
          <div>
            <div className="mb-8 flex items-center justify-between">
              ...
              <div className="flex gap-4">
                <button type="button" onClick={() => { setStep(2); setIndexJson(null); setColorVariants([]); setError(null); }}
                  className="font-mono text-xs text-muted transition-colors hover:text-accent">Regenerate</button>
                <button type="button" onClick={() => { setStep(1); setBrief(INITIAL); setScanUrl(''); setIndexJson(null); setColorVariants([]); }}
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              ...
              <div className="border border-surface-border">
                <div className="border-b border-surface-border px-5 py-3">
                  <span className="font-mono text-[11px] uppercase tracking-wider text-muted">Copy Preview</span>
                </div>
                <div className="p-5">
                  {(() => {
            ...
            <div className="mt-6 flex gap-3">
              <button type="button" onClick={() => download(indexJson, 'index.json')}
                className="flex-1 border border-accent bg-accent py-3 font-mono text-sm font-bold uppercase tracking-wider text-black transition-colors hover:bg-accent-dim">
                Download index.json
              </button>
              <button type="button" onClick={() => download(buildSettingsData(), 'settings_data.json')}
                className="flex-1 border border-surface-border py-3 font-mono text-sm uppercase tracking-wider text-muted transition-colors hover:border-white/30 hover:text-white">
