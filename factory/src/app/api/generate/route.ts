...
- footer_instagram_handle: best-guess Instagram handle for the brand, without the leading @. The scraper does not expose social handles, so infer from the brand name: lowercase, strip spaces and punctuation (e.g. "Jolene Coffee" → "jolenecoffee", "Dr. Squatch" → "drsquatch"). Return "" only if you cannot produce any plausible handle.
- footer_richtext_heading: optional heading above the footer Rich Text section. Almost always return "" — only use a 1-3 word label like "Stay Connected" if it genuinely helps layout.
- footer_richtext_content: short richtext string with an Instagram follow link, baked with the handle from footer_instagram_handle. Format EXACTLY: <p>Follow us <a href="https://instagram.com/HANDLE">@HANDLE</a> for daily inspiration</p> — substitute HANDLE with the actual handle value. Do NOT emit literal template placeholders like {{...}}. If footer_instagram_handle is "", return "".
